...
from model import Hero, Skeleton_blue, Tester, TextBox, Skeleton_red, GameManager, Warlock, Warlock_bullet, Fire, Portal, Meteor, Boss, Breath
import random
import time
    ...
    while True:
        ...
        if state == 'pause' and game.state == 'running':
            ...
            if len(breath_group) > 0:
                magic_collision(breath_group.sprite, groups)
            if len(meteor_group) > 0:
                magic_collision(meteor_group.sprite, monsters)
            meteor_group.update()
...
def magic_collision(magic, groups):
    for group in groups:
        if len(group) > 0:
            collision = pygame.sprite.spritecollide(magic, group, dokill=False, collided=pygame.sprite.collide_rect)
